...
func Physics_update():
	...
	if !interrupted and NPC.Player:
		...
		if NPC.sprite.scale.x < 0:
			NPC.sprite.offset.x = 50
			NPC.sprite.get_child(0).position.x = 50
			NPC.get_child(4).position.x = -100
		else:
			NPC.sprite.offset.x = 0
			NPC.sprite.get_child(0).position.x = 0
			NPC.get_child(4).position.x = 0
