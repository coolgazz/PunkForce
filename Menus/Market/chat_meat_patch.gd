...
const USER_CHOICES := {
	Stage.ROUND1: [
		{"code": "1a", "text": "Йо, meat.patch. Видел твой дамп. Хочу реальный профит, без шлака. Есть пруфы, что это не старый ресайклин?"},
		{"code": "1b", "text": "Слышь, мясник, цена у тебя жирная. Не хочу платить за очередной фейк. Убедишь за минуту — берусь, нет — идёшь лесом."}
	],
	Stage.ROUND2: [
		{"code": "2a", "text": "Окей, давай так: берусь по почти твоей цене, а взамен пишу честный отзыв в треде и скидываю лог боя после прошивки. Тебе лишний пруф, мне — апгрейд."},
		{"code": "2b", "text": "Да какие ещё пруфы, шей уже, пока я добрый. Я тебе ещё одолжение делаю, что вообще пишу."}
	]
}

const OPPONENT_RESPONSES := {
	"1a": "Нормально заходишь.\nХэши боёв лежат в треде, плюс отзыв от модера.\nМогу кинуть маленький фраг на одну руку — почувствуешь разницу за один спарринг.",
	"1b": "О, тяжёлый клиент подъехал.\nЛадно, пока не кинул в мут — давай, попробуй меня уговорить сам, а не угрозами.\nЕщё слово мимо — чёрный список.",
	"2a_after_1a": "Вот это базар.\nЛови скидку, плюс бонусом — пару редких паттернов на добивание.\nЗальёшь лог — подниму тебе приоритет в следующих патчах.",
	"2a_after_1b": "Ладно, умеешь не только орать.\nБез скидки, но и без наценки.\nБерёшь — шью.\nНет — дальше ору в треде.",
	"2b": "Всё, с тебя хватит.\nЧекни ник в бан-листе, может, узнаешь.\nАпгрейд тебе не светит."
}

const SUCCESS_MESSAGE := "Прошивка “Мясной патч v2.3”:\n— оптимизирует угол и скорость замаха;\n— подправляет моторные рефлексы под реальный стрит-файт;\n— даёт +1 к урону любым атакам, где замешано мясо и металл."
